            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input type="date" name="activity-date" class="form-control fonts" id="activity-date"
                        value="{{ \Carbon\Carbon::parse($engagement->date)->format('Y-m-d') }}" required>
                </div>
            </div>
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    <select class="form-control fonts" id="activity-type" name="activity-name" required>
                        <option value="Email" {{ $engagement->activity_name == 'Email' ? 'selected' : '' }}>Email</option>
                        <option value="Phone" {{ $engagement->activity_name == 'Phone' ? 'selected' : '' }}>Phone</option>
                        <option value="Meeting" {{ $engagement->activity_name == 'Meeting' ? 'selected' : '' }}>Meeting</option>
                        <option value="WhatsApp" {{ $engagement->activity_name == 'WhatsApp' ? 'selected' : '' }}>WhatsApp</option>
                    </select>
                </div>
            </div>
        </div>
            ...
            <div class="col-md-12">
                <div class="form-group">
                    <div class="attachment-container">
                        <label class="font-educ my-1" for="activity-attachment-{{ $engagement->engagement_pid }}">Attachment</label>
                        <input type="file" id="activity-attachment-{{ $engagement->engagement_pid }}" multiple accept="image/*" style="display: none;" name="activity-attachments">
                        <button type="button" class="btn hover-action" onclick="document.getElementById('activity-attachment-{{ $engagement->engagement_pid }}').click();" style="width: max-content">
                            <i class="fa-solid fa-upload"></i> Upload
                        </button>
                        <div id="file-names-{{ $engagement->engagement_pid }}" class="file-list"></div> <!-- Display filenames here -->
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    <textarea class="form-control fonts" id="activity-description" name="activity-details" rows="3" style="height: 100px;">{{ $engagement->details }}</textarea>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6">
                <label class="font-educ" for="created-date">Created Date</label>
                <input type="text" class="form-control fonts" id="created-date" value="{{ $engagement->created_at->format('Y-m-d H:i') }}" disabled>
            </div>
            <div class="col-md-6">
                <label class="font-educ" for="modified-date">Modified Date</label>
                <input type="text" class="form-control fonts" id="modified-date" value="{{ $engagement->updated_at->format('Y-m-d H:i') }}" disabled>
            </div>
        </div>
